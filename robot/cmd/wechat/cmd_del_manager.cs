...
{
    [attribute.CmdClass]
    internal class cmd_del_manager
    {
        public const string Instrus = "取消经理,移除经理,解绑经理";
        public const string Tag = "cmd_del_manager";
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(wxid))
                {
                    ...
                    else
                    {
                        ...
                        {
                            context.WechatM.SendAtText($"⚠️在取消经理时找到多个可能的群成员，昵称和id如下:\n" +
                ...
                {
                    context.WechatM.SendAtText($"{targetNick}不是本群的经理。",
                                               new List<string> { msg.WXID },
                                               msg.Self,
                                               msg.Sender);
                    return;
                ...
                var result = context.ContactsM.DelManager(msg.Self, group.WXID, wxid);
                var desc = "";
                if (result) context.WechatM.SendAtText($"@{targetNick} 已被取消本群（{club?.Name ?? "新俱乐部"}-{rid}）经理权限。\n",
                                                       new List<string> { msg.WXID },
                                                       msg.Self,
                                                       msg.Sender);
