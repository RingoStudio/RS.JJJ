            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(wxid))
                {
                    ...
                    else
                    {
                        ...
                        if (wxids is null)
                        {
                            _context.WechatM.SendAtText($"⚠️在设置成员时没有找到昵称为[{arr[1]}]的群成员。",
                                                       new List<string> { msg.WXID },
                                                       msg.Self,
                                                       msg.Sender);
                            return;
                ...
                var result = _context.ContactsM.SetMember(msg.Self, group.WXID, wxid, uid);
                if (result) _context.WechatM.SendAtText($"您已经与俱乐部 [{club?.Name ?? "新俱乐部"}-{uid}] 中 [{gameNick}] 的绑定。\n",
