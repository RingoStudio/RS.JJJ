...
{
    [attribute.CmdClass]
    internal class cmd_set_holder
    {
        public const string Instrus = "设置会长,绑定会长";
        public const string Tag = "cmd_set_holder";
        ...
        public static void Do(Context context, Message msg)
        {
            try
            {
                ...
                if (club is null)
                {
                    context.WechatM.SendAtText($"⚠️要设置的俱乐部[{rid}]不存在，请先添加俱乐部。",
                ...
                if (string.IsNullOrEmpty(wxid))
                {
                    ...
                    else
                    {
                        ...
                        if (wxids is null)
                        {
                            context.WechatM.SendAtText($"⚠️在设置会长时没有找到昵称为[{arr[1]}]的群成员。",
                                                       new List<string> { msg.WXID },
                                                       msg.Self,
                                                       msg.Sender);
                            return;
                        ...
                        {
                            context.WechatM.SendAtText($"⚠️在设置会长找到多个可能的群成员，昵称和id如下:\n" +
                ...
                var nick = context.ContactsM.QueryGroupMemberNick(wxid, msg.Self, msg.Sender);

                // 缺少用户资料
                var user = context.ContactsM.FindUser(msg.Self, wxid);
                if (user is null)
                {
                    context.WechatM.SendAtText($"⚠️唧唧叽缺少[{nick}]的资料，请联系会长使用命令\"刷新群信息\"。",
                                                                         new List<string> { msg.WXID },
                                                                         msg.Self,
                                                                         msg.Sender);
                    return;
                ...
                if (!string.IsNullOrEmpty(curHolder))
                {
                    ...
                    if (curHolder == wxid)
                    {
                        context.WechatM.SendAtText($"[{nick}]已经是俱乐部[{club?.Name ?? "新俱乐部"}-{rid}]的会长。",
                                                                            new List<string> { msg.WXID },
                                                                            msg.Self,
                                                                            msg.Sender);
                        return;
                    ...
                    {
                        var curNick = context.ContactsM.QueryGroupMemberNick(curHolder, msg.Self, msg.Sender);
                        if (string.IsNullOrEmpty(curNick)) curNick = curHolder;
                        context.WechatM.SendAtText($"⚠️俱乐部[{club?.Name ?? "新俱乐部"}-{rid}]目前的会长是[{curNick}]。\n" +
                ...
                var result = context.ContactsM.SetHolder(msg.Self, rid, wxid);
                var desc = "";
                if (result) context.WechatM.SendAtText($"您已经被指派俱乐部[{club?.Name ?? "新俱乐部"}-{rid}]会长权限。\n" +
