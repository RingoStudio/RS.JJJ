...
namespace RS.Snail.JJJ.robot.cmd.mayday
{
    ...
    internal class cmd_gen_recipe : ICMD
    {
        public Context _context { get; set; }
        public cmd_gen_recipe(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "生成处方" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_gen_recipe";
        ...
        private void OnMessageArrival(Message msg)
        {
            try
            {
                ...
                else if (msg.Content == "确定")
                {
                    ...
                    {
                        SendRecipe(msg, token);
                        ClearMission(msg);
                    }
                }
                else
                {
                    var result = _context.MaydayM.ImportRecipeTokenFromText(msg.Content, ref token);
                    if (!result.anyMarks)
                    {
                        Loop(msg, true);
                    }
                    ...
                    {
                        if (result.result) _context.WechatM.SendAtText("请回复\"确定\"生成处方，或继续修改以上信息。", new List<string> { msg.Sender }, msg.RoomID);
        ...
        private void ClearMission(Message msg)
        {
            try
            {
                ...
                _context.CommunicateM.UnregistWaitMessageRequest(msg.RoomID, msg.Sender, Tag, false);
            }
            catch (Exception ex)
            {
                Context.Logger.WriteException(ex, Tag);
        ...
        private void SendRecipe(Message msg, MaydayTicketToken token)
        {
            try
            {
                var files = _context.MaydayM.DrawRecipe(token, msg);
                if (files.Count <= 0)
                {
                    _context.WechatM.SendAtText("因未知原因，处方生成失败了。",
                                                new List<string> { msg.Sender },
                                                msg.RoomID);
                }
                else
