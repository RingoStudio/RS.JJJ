...
{
    [attribute.CmdClass]
    internal class cmd_broadcast_public
    {
        public const string Instrus = "公共广播";
        public const string Tag = "cmd_broadcast_public";
        ...
        public static void Do(Context context, Message msg)
        {
            try
            {
                ...
                context.WechatM.SendAtText("请发送\"确定\"立即向所有唧唧叽所在的群发出以上广播内容。\n" +
                                           "若想要添加文本/文件/图片，请在此发送。\n" +
        ...
        private static void SendBroadCasts(Context context, Message msg, List<string> texts, List<string> images, List<string> files)
        {
            try
            {
                ...
                foreach (var chatroom in context.ContactsM.GetAllGroupWXID(msg.Self, false))
                {
                    context.WechatM.SendText(text, msg.Self, chatroom);
                    foreach (var path in images)
                    {
                        context.WechatM.SendImage(path, msg.Self, chatroom);
                    }
                    foreach (var path in files)
                    {
                        context.WechatM.SendFile(path, msg.Self, chatroom);
                    }
                }
            }
            catch (Exception ex)
