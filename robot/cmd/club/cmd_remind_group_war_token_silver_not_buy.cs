...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_remind_group_war_token_silver_not_buy : ICMD
    {
        public Context _context { get; set; }
        public cmd_remind_group_war_token_silver_not_buy(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "提醒银牌未氪", "提醒银令牌未氪", "提醒氪银牌", "提醒氪银令牌" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_remind_group_war_token_silver_not_buy";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    var result = _context.ClubsM.RemindGroupWarTokenSilverNotBuy(msg.Self, rid, msg.Sender, msg.WXID);
                    if (!result.result)
                    {
