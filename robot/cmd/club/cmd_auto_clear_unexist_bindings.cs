                      ...
                      foreach (var member in group.Members)
                      {
                          if (member.Value.UIDs is not null && member.Value.UIDs.Count > 0)
                          {
                              member.Value.UIDs = member.Value.UIDs.Where((uid) =>
                              {
                                  ...
                                  if (!flag) ret.Add($"@{member.Value.NickName} -> {_context.ClubsM.QueryMemberName(msg.Self, uid) ?? ""}[{uid}]");
                                  return flag;
                              }).ToList();
                              ...
                              if (member.Value.UIDs.Count <= 0 && member.Value.Role < include.UserRole.GROUP_MANAGER)
                              {
                                  ...
                                  _context.ContactsM.SetRoleCache(include.UserRole.NONE, member.Key, msg.Self, msg.Sender);
                              }
                          }
                      }
