...
{
    [attribute.CmdClass]
    internal class cmd_query_group_war_events
    {
        public const string Instrus = "查询物种事件,查询事件";
        public const string Tag = "cmd_query_group_war_events";
            ...
            try
            {
                ...
                if (arr.Length > 1)
                {
                    for (int i = 1; i <= arr.Length; i++)
                    {
                        ...
                        {
                            var t = context.GroupWarEventsM.ConvertEventType(arr[i]);
                            if (t > 0) type = t;
                        }
                    }
                }
                ...
                var result = context.GroupWarEventsM.QueryEventsDesc(msg.Self, rid, area, type);
                if (string.IsNullOrEmpty(result)) context.WechatM.SendAtText("⚠️未查询到任何信息。",
                                                                            new List<string> { msg.WXID },
                                                                            msg.Self,
                                                                            msg.Sender);
                else context.WechatM.SendAtText(result,
                                                new List<string> { msg.WXID },
                                                msg.Self,
                                                msg.Sender);
            }
