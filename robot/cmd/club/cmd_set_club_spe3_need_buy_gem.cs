...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_set_club_spe3_need_buy_gem : ICMD
    {
        public Context _context { get; set; }
        public cmd_set_club_spe3_need_buy_gem(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "设置北极贝需要买卷轴" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_set_club_spe3_need_buy_gem";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                club.Spe3DontNeedBuyGem = (mode < 0);
                var desc = $"已将俱乐部[{club.Name}]的北极贝买卷轴设置为";
                desc += (!club.Spe3DontNeedBuyGem) ? "[需要]\n该俱乐部未使用白蝌蚪购买卷轴的成员将不会被提醒" :
                                               "[不需要]\n该俱乐部未使用白蝌蚪购买卷轴的成员将会被提醒";
