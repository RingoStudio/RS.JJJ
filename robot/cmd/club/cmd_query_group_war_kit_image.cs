...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_query_group_war_kit_image : ICMD
    {
        public Context _context { get; set; }
        public cmd_query_group_war_kit_image(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "生成布阵图" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_query_group_war_kit_image";
        ...
        public void Do(Message msg)
        {
            try
            {
                ...
                {
                    if (StringHelper.IsRID(item)) rid = item;
                    else if (item == "强制" || item.ToLower() == "force") force = true;
                }
                ...
                if (force)
                {
                    if (_context.ContactsM.QueryRole(msg.Sender, "") < UserRole.ADMINISTRATOR) force = false;
                }
                ...
                var result = _context.ClubsM.GetClubKitPlanImage(rid, force);
