...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_query_group_war_boss_combat_record_excel : ICMD
    {
        public Context _context { get; set; }
        public cmd_query_group_war_boss_combat_record_excel(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "查询boss伤害总览" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_query_group_war_boss_combat_record_excel";
        ...
        public void Do(Message msg)
        {
            try
            {
                ...
                {
                    _context.WechatM.SendAtText($"您没有查看该俱乐部相关信息的权限。",
                                                new List<string> { msg.Sender }, msg.RoomID);
                    return;
                }
                ...
                var result = _context.ClubsM.QueryGroupWarBossCombatRecordExcel(rid);
