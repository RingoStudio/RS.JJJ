...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_set_club_remind_group_member_changed : ICMD
    {
        public Context _context { get; set; }
        public cmd_set_club_remind_group_member_changed(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "设置提醒微信群成员变动", "提醒微信群成员变动" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_set_club_remind_group_member_changed";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                club.DontRemindGroupMemberChanged = (mode < 0);
                var desc = $"已将俱乐部[{club.Name}]的提醒微信群成员变动设置为";
                desc += (!club.DontRemindGroupMemberChanged) ? "[开启]\n之后每一次刷新群信息，将向俱乐部群发送成员变动名单。" :
                                                               "[关闭]\n之后不会自动发出微信群成员变动提醒。";
