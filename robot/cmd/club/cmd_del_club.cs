    ...
    internal class cmd_del_club : ICMD
    {
        ...
        public void Do(Message msg)
        {
            ...
            try
            {
                ...
                if (club is null)
                {
                    _context.WechatM.SendAtText($"⚠️要移除的俱乐部 [{rid}] 不存在！", new List<string> { msg.Sender }, msg.RoomID);
                    return;
                }
                ...
                _context.CommunicateM.RegistWaitMessageRequest(msg.RoomID, msg.Sender,
                                                             onReceivedCallback: new Action<Message>((_msg) =>
                                                             {
                                                                 ...
                                                                 try
                                                                 {
                                                                     result = _context.ClubsM.DelClub(rid);
                                                                     if (result) result = _context.ContactsM.UnbindGroup(_msg.Sender);
                                                                     if (result) result = _context.ContactsM.DelHolder(rid);
                                                                 }
                                                                 catch (Exception ex)
                                                                 {
