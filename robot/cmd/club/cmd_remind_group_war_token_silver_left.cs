...
{
    [attribute.CmdClass]
    internal class cmd_remind_group_war_token_silver_left
    {
        public const string Instrus = "提醒清银牌,提醒清银令牌,提醒用银牌,提醒用银令牌";
        public const string Tag = "cmd_remind_group_war_token_silver_left";
        ...
        public static void Do(Context context, Message msg)
        {
            try
            {
                ...
                var result = context.ClubsM.RemindGroupWarTokenSilverLeft(msg.Self, rid, msg.Sender, msg.WXID);
                if (!result.result)
                {
