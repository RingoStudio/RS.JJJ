...
{
    [attribute.CmdClass]
    internal class cmd_remind_group_war_token_gold_left
    {
        public const string Instrus = "提醒清金牌,提醒清金令牌,提醒用金牌,提醒用金令牌";
        public const string Tag = "cmd_remind_group_war_token_gold_left";
        ...
        public static void Do(Context context, Message msg)
        {
            try
            {
                ...
                var result = context.ClubsM.RemindGroupWarTokenGoldLeft(msg.Self, rid, msg.Sender, msg.WXID);
                if (!result.result)
                {
