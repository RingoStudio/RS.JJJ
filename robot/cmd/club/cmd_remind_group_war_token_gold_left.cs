...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_remind_group_war_token_gold_left : ICMD
    {
        public Context _context { get; set; }
        public cmd_remind_group_war_token_gold_left(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "提醒清金牌", "提醒清金令牌", "提醒用金牌", "提醒用金令牌" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_remind_group_war_token_gold_left";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    var result = _context.ClubsM.RemindGroupWarTokenGoldLeft(msg.Self, rid, msg.Sender, msg.WXID);
                    if (!result.result)
                    {
