...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_query_group_war_token_silver_not_use : ICMD
    {
        public Context _context { get; set; }
        public cmd_query_group_war_token_silver_not_use(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "查询银牌未用", "查询银令牌未用", "查询银牌剩余", "查询银令牌剩余" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_query_group_war_token_silver_not_use";
        ...
        public void Do(Message msg)
        {
            try
            {
                ...
                var result = _context.ClubsM.QueryGroupWarSilverTokeNotUse(rid);
                if (string.IsNullOrEmpty(result)) _context.WechatM.SendAtText("⚠️未查询到任何信息。", new List<string> { msg.Sender }, msg.RoomID);
                else if (result.Length > 200)
                {
                    var fileName = $"OUT\\银牌剩余查询结果_@{_context.ContactsM.QueryGroupMemberNickForFile(msg.Sender, msg.RoomID)}_{DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss")}.txt";
