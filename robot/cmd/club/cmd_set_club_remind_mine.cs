...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_set_club_remind_mine : ICMD
    {
        public Context _context { get; set; }
        public cmd_set_club_remind_mine(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "设置登录后提醒挖矿", "设置自动提醒挖矿" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_set_club_remind_mine";
        ...
        public void Do(Message msg)
        {
            try
            {
                ...
                club.DontRemindMine = (mode < 0);
                var desc = $"已将俱乐部[{club.Name}]的登录后提醒挖矿设置为";
                desc += (!club.DontRemindMine) ? "[开启]\n下一次登录之后，将自动发出挖矿相关提醒" :
                                                 "[关闭]\n下一次登录之后，将不会自动发出挖矿相关提醒\n但可以手动发出提醒";
