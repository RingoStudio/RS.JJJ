...
{
    internal class cmd_remind_group_war_clear_resource : ICMD
    {
        public Context _context { get; set; }
        public cmd_remind_group_war_clear_resource(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "提醒清资源" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_remind_group_war_clear_resource";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    var result = _context.ClubsM.RemindGroupWarUseResources(msg.Self, rid, msg.Sender, msg.WXID);
                    if (!result.result)
                    {
