...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_set_club_login_dont_at_holder : ICMD
    {
        public Context _context { get; set; }
        public cmd_set_club_login_dont_at_holder(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "设置登录提示会长", "设置登录at会长" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_set_club_login_dont_at_holder";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                club.LoginDontAtHolder = (mode < 0);
                var desc = $"已将俱乐部[{club.Name}]的登录提醒会长设置为";
                desc += (!club.LoginDontAtHolder) ? "[开启]" : "[关闭]";
