...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_remind_group_war_drill : ICMD
    {
        public Context _context { get; set; }
        public cmd_remind_group_war_drill(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "提醒钻头" };
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    var result = _context.ClubsM.RemindGroupWarDrill(msg.Self, rid, msg.Sender, msg.WXID, min, notice);
                    if (!result.result)
                    {
