...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_remind_group_war_history : ICMD
    {
        public Context _context { get; set; }
        public cmd_remind_group_war_history(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "提醒物种历史", "提醒历史" };
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    var result = _context.ClubsM.RemindGroupWarHistory(msg.Self, rid, msg.Sender, msg.WXID);
                    if (!result.result)
                    {
