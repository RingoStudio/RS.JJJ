...
{
    [attribute.CmdClass]
    internal class cmd_query_group_war_events_image
    {
        public const string Instrus = "生成事件表,生成事件图";
        public const string Tag = "cmd_query_group_war_events_image";
        ...
        public static void Do(Context context, Message msg)
        {
            try
            {
                ...
                var result = context.ClubsM.GetGroupWarEventsMultiImage(msg.Self, rid);
                if (string.IsNullOrEmpty(result) || !System.IO.File.Exists(result)) context.WechatM.SendAtText("⚠️生成失败。",
                                                                            new List<string> { msg.WXID },
                                                                            msg.Self,
                                                                            msg.Sender);
                else context.WechatM.SendImage(result, msg.Self, msg.Sender);
