...
namespace RS.Snail.JJJ.robot.cmd.club
{
    ...
    internal class cmd_query_group_war_events_image : ICMD
    {
        public Context _context { get; set; }
        public cmd_query_group_war_events_image(Context context)
        {
            _context = context;
        }
        public List<string> Commands => new List<string> { "生成事件表", "生成事件图" };
        public List<string> CommandsJP { get => Commands.Select(a => Pinyin.GetInitials(a).ToLower()).ToList(); }
        public List<string> CommandsQP { get => Commands.Select(a => Pinyin.GetPinyin(a).ToLower()).ToList(); }
        public string Tag => "cmd_query_group_war_events_image";
        ...
        async public Task Do(Message msg)
        {
            try
            {
                ...
                var result = await Task.Run(() => _context.ClubsM.GetGroupWarEventsMultiImage(msg.Self, rid));
